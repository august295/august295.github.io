...
答：因为初始化 `right` 的赋值是 `nums.size() - 1`，即最后一个元素的索引，而不是 `nums.size()`。这二者可能出现在不同功能的二分查找中，区别是：前者相当于两端都闭区间 `[left, right]`，后者相当于左闭右开区间 `[left, right)`，因为索引大小为 `nums.size()` 是越界的。文中所有的算法都是基于前者 `[left, right]` 两端都闭的区间。**这个区间其实就是每次进行搜索的区间**。
