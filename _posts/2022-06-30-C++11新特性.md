...
2: Store	reg4, a;	# 将寄存器reg4中的数据存入内存地址a
2: Loadi	reg5, 2;	# 将立即数2放入寄存器reg5
2: Store	reg5, b;	# 将寄存器reg5中的数据存入内存地址b
```

这里我们演示了 `t = l;a = t;b = 2;` 这段C++语言代码的伪汇编表示。按照通常的理 解，指令总是按照 `1->2->3->4->5` 这样顺序执行，如果处理器的执行顺序是这样的话，我们通常称这样的内存模型为强顺序的（strong ordered ）。
