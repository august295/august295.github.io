...
![image-20221204182054971](/media/image/2022-12-04-格式化代码C++/settings-vscode.png)
...
![image-20221204182835579](/media/image/2022-12-04-格式化代码C++/shortcut-vscode.png)
...
![image-20221204192856072](/media/image/2022-12-04-格式化代码C++/settings-vs.png)

格式化的快捷方式 `Ctrl+K Ctrl+D`，修改在工具 => 选项 => 环境 => 键盘，修改 `编辑.设置文档格式` 格式，格式化的快捷方式修改为 `Shift+Alt+F`。

![image-20221204193818574](/media/image/2022-12-04-格式化代码C++/shortcut-vs.png)

## 2.3 Qt Creator 4.11

`Qt Creator 4.11` 安装好也是自带格式化设置，但是需要打开。在帮助 => 关于插件 => `C++` => `Beautifier(expermental)` 打开重启即可。

![image-20221204195156228](/media/image/2022-12-04-格式化代码C++/settings-qt-open.png)

然后设置 => 工具 => 选项 => `Beautifier` => `Clang Format`，然后设置格式化程序和格式化文件，`Use customized style` 点击 `Edit` 将格式化文件复制到里面即可（不支持中文，所以需要删除注释）。

![image-20221204202654503](/media/image/2022-12-04-格式化代码C++/settings-qt.png)

添加格式化快捷键在工具 => 选项 => 环境 => 键盘，修改 `FormatFile` 格式，点击 `Record` 记录修改后的快捷键，格式化的快捷方式修改为 `Shift+Alt+F`。

![image-20221204203121674](/media/image/2022-12-04-格式化代码C++/shortcut-qt.png)
