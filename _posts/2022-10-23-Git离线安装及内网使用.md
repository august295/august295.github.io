...
仓库分为裸露（bare）和非裸露[^1]，`bare` 模式只保存文件操作历史记录，仓库中是看不到具体文件，只有下拉后才能看到。
...
| / 后置 | 匹配项目和目录                                                                                                                                             |
|        | 如果在模式的开头或中间（或两者）有一个分隔符，那么该模式是相对于特定`.gitignore`文件本身的目录级别的。否则模式也可以在该级别以下的任何级别匹配`.gitignore` |
| ?      | 匹配除 `/` 之外的任何一个字符                                                                                                                              |
| []     | 匹配单个字符列表                                                                                                                                           |
| *      | 匹配除 `/` 以外的任何内容                                                                                                                                  |
| **     | 表示目录所有，可以前置、中间、后置                                                                                                                         |
...
[^1]: [Git 本地仓库和裸仓库](https://zhuanlan.zhihu.com/p/24151683)
