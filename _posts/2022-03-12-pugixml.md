...
本文都是根据官方文件 [https://pugixml.org/docs/manual.html](https://pugixml.org/docs/manual.html) 翻译并只保留基本介绍和接口说明，版本适用于 `V1.12`。
...
（1）pugixml的三个文件，可以只include头文件pugixml.hpp，CPP文件不用放到项目中，方法是在pugiconfig.hpp中：
...
（2）如果项目使用的是Unicode设置，则可以在pugiconfig.hpp中：
...
void demo2()
{
    ...
    pugi::xml_document doc;
    // 默认 "rb" 加载文档
    pugi::xml_parse_result result = doc.load_file("read.xml");
    if (!result)
    {
        std::cout << "load fail..." << std::endl;
        return;
