...
categories: 交叉编译
tags: qt5.15.2 交叉编译 loongarch64
author: August
mathjax: true
typora-root-url: ..
...
...
该篇文章都是基于上一篇文章 [qt5.15.2交叉编译arm64.md](2024-04-30-qt5.15.2交叉编译.md) 修改。
...
wget -c https://www.linuxfromscratch.org/patches/blfs/svn/nss-3.98-standalone-1.patch
...
未打上补丁地方之一，在 `./qtbase/src/3rdparty/double-conversion/include/double-conversion/utils.h` 96 行修改
...
根据提示修改 `/qtscript/src/3rdparty/javascriptcore/JavaScriptCore/runtime/JSValue.h` 493 和 505 行 `int32_t` 改为 `int64_t`。
...
补丁没打完全，loongarch64 不支持 dav1d，修改 `/qtwebengine/src/3rdparty/chromium/media/media_options.gni`
...
使用 `simplebrowser.pro` 示例进行测试。可以看到最终编译结果是 `loongarch64` 版本。
