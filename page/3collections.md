---
layout: page
title: Collections
permalink: /collection/
icon: dropbox
type: page
---

* content
{:toc}


## 1 工具

### 1.1 开发工具

#### 1.1.1 Visual Studio

| 分类         | 名称                        | 介绍                                                     | 官网                                             | 备注          |
| :----------- | --------------------------- | -------------------------------------------------------- | ------------------------------------------------ | ------------- |
| 集成开发环境 | Visual Studio（VS）         | Windows下进行最建议使用的C++编程软件                     | <https://visualstudio.microsoft.com/zh-hans/vs/> |               |
| 插件         | Visual Assist X（番茄助手） | VS插件，能够实现高亮显示、智能代码提示、显示编码错误     | <https://www.wholetomato.com/>                   | [1](#1.1.1.1) |
| 插件         | AtomineerUtils              | VS插件，国外的一款用于生成源代码注释的工具，支持多种风格 | <https://www.atomineerutils.com/>                | [2](#1.1.1.2) |

<p id="1.1.1.1"> [1] 链接：https://pan.baidu.com/s/1hNlw_ZssO42m-QPYE8jYxg 提取码：lzjo 最后支持VS2019 </p>

<p id="1.1.1.2"> [2] 链接：https://pan.baidu.com/s/1oAyPsgHo41GduR44-ruRng 提取码：77cc 最后支持VS2017 </p>



#### 1.1.2 Visual Studio Code

| 分类   | 名称                            | 介绍                         | 备注                                                      |
| :----- | ------------------------------- | ---------------------------- | --------------------------------------------------------- |
| 编辑器 | Visual Studio Code（VSCode）    | 微软一款轻量级跨平台的编辑器 | <https://code.visualstudio.com/>                          |
| 插件   | Chinese (Simplified) (简体中文) | 将VSCode进行汉化             |                                                           |
| 插件   | C/C++                           | 能够实现C/C++的检查          | 自带格式化，并且能够配合自定义 `.clang-format` 实现格式化 |
| 插件   | Doxygen Documentation Generator | 用于生成源代码注释的工具     | 可定制化                                                  |

### 1.2 其他工具

#### 1.2.1 搜索工具

| 名称       | 介绍                                             | 官网                               |
| ---------- | ------------------------------------------------ | ---------------------------------- |
| Everything | 一款基于名称快速定位文件和文件夹的轻量级搜索工具 | <https://www.voidtools.com/zh-cn/> |

#### 1.2.2 Markdown

| 名称   | 介绍                         | 官网                 | 备注 |
| ------ | ---------------------------- | -------------------- | ---- |
| Typora | 一款 Markdown 编辑器和阅读器 | https://typoraio.cn/ |      |



## 2 三方库

### 2.1 JSON库

| 名称  | 官方网址 | 开源网址                              |
| ----- | -------- | ------------------------------------- |
| cjson |          | <https://github.com/DaveGamble/cJSON> |

### 2.2 XML库

| 名称    | 官方网址               | 开源网址                          |
| ------- | ---------------------- | --------------------------------- |
| pugixml | <https://pugixml.org/> | <https://github.com/zeux/pugixml> |

### 2.3 SQL库

| 名称   | 官方网址                        | 开源网址                           |
| ------ | ------------------------------- | ---------------------------------- |
| sqlite | <https://sqlite.org/index.html> | <https://github.com/sqlite/sqlite> |



## 3 有用网址

| 分类        | 介绍                    | 网址                                |
| ----------- | ----------------------- | ----------------------------------- |
| 汇编        | C++代码转换为汇编代码   | <https://godbolt.org/>              |
| C++参考手册 | 中文版，C++官方参考手册 | <https://zh.cppreference.com/w/cpp> |
| C++参考手册 | 英文版，C++官方参考手册 | <https://en.cppreference.com/w/cpp> |



## Comments

{% include comments.html %}
